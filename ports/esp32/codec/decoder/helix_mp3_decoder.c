...
mp3_decode_t *mp3_decode_init()
{
    ...
    mp3_decode_t *decoder = calloc(1, sizeof(mp3_decode_t));
    if(decoder == NULL)
        mp_raise_ValueError("Memory not enough for mp3 decode handle.");
    decoder->readBuf = malloc(MAINBUF_SIZE1);
    if (decoder->readBuf == NULL){
        free(decoder);
        mp_raise_ValueError("Memory not enough for mp3 readBuf.");
    }
    decoder->output = malloc(1153 * 4);
    if (decoder->output == NULL){
        free(decoder->readBuf);
        free(decoder);
         mp_raise_ValueError("Memory not enough for mp3 outpuf buff.");
    }
